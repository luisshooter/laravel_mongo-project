        ...
        <div class="col-lg-8 offset-lg-2">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h5 class="mb-0"><i class="bi bi-receipt"></i> Pedido #{{ substr($order->_id, -6) }} - Cliente:
                        {{ $order->customer_name ?? 'Não Informado' }}</h5>
                ...
                <div class="card-body">
                        ...
                        <tbody>
                            @foreach ($order->items ?? [] as $it)
                                <tr>
                                    ...
                                    <td>R$
                                        {{ number_format(($it['quantity'] ?? 0) * ($it['unit_price'] ?? 0), 2, ',', '.') }}
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                    <p class="text-end mb-0"><strong>Total: R$
                            {{ number_format($order->total_price, 2, ',', '.') }}</strong></p>
                    <p class="text-end mt-1 mb-0"><span class="text-muted">Forma de pagamento:</span>
                    ...
                    <div class="mt-4 pt-3 border-top">
                        ...
                        @endif
                        <p class="mb-1"><strong>Cliente:</strong> {{ $order->customer_name ?? 'Não Informado' }}</p>
                        <p class="mb-1"><strong>CPF:</strong> {{ $order->customer_cpf ?? 'Não Informado' }}</p>
                        <p class="mb-1"><strong>Endereço:</strong> {{ $order->customer_address ?? 'Não Informado' }}</p>
                        <p class="mb-1"><strong>Data:</strong> {{ $order->formatted_created_at }}</p>
                    </div>

                    <hr>
                    <div class="d-flex justify-content-between">
                        <a href="{{ route('orders.index') }}" class="btn btn-secondary"><i class="bi bi-arrow-left"></i>
                            Voltar</a>
                        <div>
